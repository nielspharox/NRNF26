...
function emailHtml(groupName: string, inviteUrl: string, inviterName: string) {
  const who = inviterName ? esc(inviterName) : "iemand";
  const grp = groupName ? `het complot <b>${esc(groupName)}</b>` : "een complot";
  return `<!doctype html><html><body style="margin:0;background:#1a0033;padding:24px;font-family:Arial,Helvetica,sans-serif">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
    <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="max-width:480px;background:#2a0050;border:2px solid #5500aa">
      ...
      <tr><td style="padding:28px 28px 8px;text-align:center">
        ...
        <div style="color:#aa44ff;font-size:13px;margin-top:4px">WK POOL 2026</div>
      </td></tr>
      <tr><td style="padding:8px 28px;text-align:center">
        <h1 style="color:#f0e6ff;font-size:24px;margin:14px 0">${who} daagt je uit! ⚽</h1>
        <p style="color:#d4bef5;font-size:17px;line-height:1.5;margin:0 0 8px">
          Je bent uitgenodigd voor ${grp} in de WK-pool waar lef beloond wordt.
          Hoe groter de underdog, hoe dikker de punten. Iedereen tipt de favoriet — jij niet.
        </p>
      </td></tr>
      <tr><td style="padding:18px 28px;text-align:center">
        <a href="${esc(inviteUrl)}" style="display:inline-block;background:#00ff66;color:#0f001f;font-weight:bold;font-size:16px;text-decoration:none;padding:14px 28px;border:2px solid #00cc44">DOE MEE ▶</a>
  ...
  try {
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        html: emailHtml(body.groupName || "", body.inviteUrl || "", body.inviterName || ""),
      }),
    });
